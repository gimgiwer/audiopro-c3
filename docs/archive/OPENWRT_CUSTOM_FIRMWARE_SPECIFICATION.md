...
6. **Нативный ASoC драйвер звука MT7688 (`kmod-sound-mt7620` / `835-asoc`):**
   - Нативная поддержка I2S DMA в ядре Linux 5.15 для MediaTek MT7688 с корректным сбросом буферов и управлением аппаратными регистрами через `regmap`.
7. **Совместимость с U-Boot Linkplay (`-O svr4`):**
   - Сборка с параметром `-O svr4` (OS ID 20) обеспечивает 100% совместимость.
8. **MIPS 24KEc компиляторные флаги:**
   - Компиляция с `-march=24kc -mtune=24kc -msoft-float` для максимального быстродействия без эмуляции инструкций.
...
| :--- | :--- | :--- |
| [**`tftp_root/openwrt.bin`**]((local path removed)) | **6.93 MB** | **Целевой образ для TFTP RAM Boot** (загрузка в оперативную память без прошивки) |
| [`audiopro-c3/bin/openwrt-ramips-mt76x8-audiopro_c3-initramfs-kernel.bin`]((local path removed)) | 6.93 MB | Резервная копия initramfs |
| [`audiopro-c3/bin/openwrt-ramips-mt76x8-audiopro_c3-squashfs-sysupgrade.bin`]((local path removed)) | 7.1 MB | Образ для постоянной прошивки во Flash через `sysupgrade` или `mtd write` |
